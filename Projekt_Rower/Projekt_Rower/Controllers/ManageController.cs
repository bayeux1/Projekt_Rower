...
namespace Projekt_Rower.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        {
            var userId = User.Identity.GetUserId();
            var userdata = _profileService.GetData(userId);
            return PartialView(@"~/Views/Manage/UserData.cshtml", userdata);
        }

        public ActionResult UserEdit()
        {       
            var userId = User.Identity.GetUserId();
            var user = _profileService.GetUserData(userId);
            if (user == null)
            {
                return HttpNotFound();
            }
        ...
        [HttpPost]
        public ActionResult UserEdit(Uzytkownicy user)
        {
            _profileService.UserProfileEdit(user);
            return RedirectToAction("Index");
        }
